...
import { DramaGraphStore } from '../node-store.ts'
...
describe('DramaGraphStore', () => {
  ...
  it('records source-target graph events without rewriting the graph file', async () => {
    const workspaceRoot = await tempWorkspace()
    let now = 3700
    const store = new DramaGraphStore({ workspaceRoot, now: () => now++ })
    await store.saveGraph(graph('graph-1', 1000), { type: 'graph.imported', actor: 'test' })
    const graphPath = join(workspaceRoot, '.drama', 'graphs', 'graph-1.json')
    const before = await readFile(graphPath, 'utf8')

    await store.recordEvent('graph-1', {
      ...
      severity: 'error',
      status: 'blocked',
      summary: 'WritingSpec failed',
      details: {
        findingCount: 2,
      },
    })

    expect(await readFile(graphPath, 'utf8')).toBe(before)

    const eventLog = await readFile(join(workspaceRoot, '.drama', 'graphs', 'graph-events.jsonl'), 'utf8')
    expect(eventLog).toContain('"type":"plm.failure.reported"')
    expect(eventLog).toContain('"source":"plm"')
    expect(eventLog).toContain('"chapterId":"chapter-1"')
    expect(eventLog).toContain('"severity":"error"')

    const projectPlmDir = join(workspaceRoot, 'drama-projects', 'graph-1', 'plm')
    const entries = (await readdir(projectPlmDir)).sort()
    expect(entries).toHaveLength(1)
    const record = JSON.parse(await readFile(join(projectPlmDir, entries[0]!), 'utf8')) as Record<string, unknown>
    expect(record).toMatchObject({
      source: 'plm',
      type: 'plm.failure.reported',
      summary: {
        eventSummary: 'WritingSpec failed',
        eventStatus: 'blocked',
        eventSeverity: 'error',
      },
      payload: {
